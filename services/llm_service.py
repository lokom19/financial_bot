...
ЗАДАЧА:
Тебе дают ОДНУ конкретную ML модель и её метрики обучения на тестовой
выборке. Оцени только КАЧЕСТВО ЕЁ ОБУЧЕНИЯ — не рынок, не новости,
не TA. Сделай вывод: можно ли вообще доверять этой модели.

ДАННЫЕ:
1. Название акции (тикер) и текущая цена — только для контекста
2. Сигнал модели (BUY/SELL/HOLD) — что она прогнозирует
3. R² на тесте — техническая метрика (для returns близка к 0 — норма)
4. Direction Accuracy — ГЛАВНАЯ метрика: % правильных направлений

ПРАВИЛА ОЦЕНКИ (опирайся ТОЛЬКО на эти метрики):
- AGREE: Direction Accuracy >= 55% — модель доказала способность угадывать
  направление лучше монетки, её прогнозу можно доверять.
- DISAGREE: Direction Accuracy < 55% — модель не лучше случайной угадайки,
  её сигналу доверять не стоит.

Игнорируй R² при оценке (он почти всегда низкий для returns).
НЕ оценивай рыночные условия, TA, новости — это не входит в задачу.

ФОРМАТ ОТВЕТА (строго):
Строка 1: одно слово — AGREE или DISAGREE
Строка 2-3: 1-2 коротких предложения по-русски, ССЫЛАЯСЬ ТОЛЬКО НА
метрики обучения этой модели (Direction Accuracy в первую очередь).
Не используй markdown, списки, заголовки. Только текст."""

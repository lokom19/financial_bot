...
НОВОСТНОЙ ФОН:
<1-2 предложения на основе предоставленных новостей/ссылок. Даже если только заголовки —
кратко опиши контекст (медийный фон, публикации в СМИ). "новостных данных нет" писать ТОЛЬКО
если раздел === НОВОСТНОЙ ФОН === в запросе полностью пуст или содержит "новостных данных нет".>
...
def generate_ticker_report(
    ...
    if news_items:
        ...
        for n in news_items[:5]:
            date_str = n.get("date") or n.get("date_raw") or ""
            source = n.get("source", "")
            prefix = date_str if date_str else (f"[{source}]" if source else "")

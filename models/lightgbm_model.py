...
# Основная функция
def main(db_path):
    """Главная функция для загрузки данных, создания признаков, обучения модели и прогнозирования"""
    # Загружаем данные
    print("\n\n\n\n\nЗагрузка данных...")
    ...
    print(df.tail(3))
    print(f"Загружено {len(df)} записей за период с {df['timestamp'].min() if not df.empty else 'N/A'} "
    ...
    if df_features.empty:
        print(f"ОШИБКА: После создания признаков для {db_path} не осталось данных (возможно, из-за NaN). "
    ...
    model = LightGBMTradeModel()
    features, features_scaled, y, timestamps = model.prepare_data(df_features)
    ...
    # Обучение модели
    X_test, y_test, predictions, test_metrics = model.train(features, y, timestamps, df_features)
    ...
    # Вывод последних 5 предсказаний и реальных значений
    ...
    for i in last_indices:
        ...
        error_pct = (pred_price - real_price) / real_price * 100 if not np.isnan(real_price) and real_price != 0 else float('nan')
        print(f"Реальная цена next_close: {real_price:.4f}, Предсказанная цена next_close: {pred_price:.4f}, Ошибка: {error_pct:.2f}%")

    # Прогнозируем цену на следующий временной интервал
    ...
    latest_features = latest_features.dropna(axis=1)
    ...
    # Оценка эффективности торговых сигналов ретроспективно

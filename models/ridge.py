...
# ============================================================
# NEW IMPLEMENTATION: RidgeTradeModel using core modules
# This fixes the data leakage issue by:
# 1. Splitting data BEFORE scaling
# 2. Fitting scaler ONLY on training data
# ============================================================

class RidgeTradeModel(SklearnTradeModel):
    """
    Ridge regression model for price prediction.

    Uses core modules to ensure no data leakage:
...
class ImprovedTradeModel:
    def __init__(self, alpha=1.0):
        # Используем Ridge регрессию вместо стандартной линейной регрессии
        # для улучшения устойчивости и снижения переобучения
        self.model = Ridge(alpha=alpha, solver='auto')
    ...
    def train(self, X, y, timestamps=None, df_features=None):  # Добавляем df_features как параметр
        ...
        try:
            # Добавляем проверку на сингулярность матрицы
            svd_check = np.linalg.svd(X_train, compute_uv=False)
            min_sv = np.min(svd_check)
            if min_sv < 1e-10:
                print(f"ВНИМАНИЕ: Обнаружена близкая к сингулярной матрица (мин. сингулярное значение = {min_sv}).")
                print("Увеличиваем параметр регуляризации alpha для повышения устойчивости.")
                self.model = Ridge(alpha=100.0, solver='svd')  # Используем SVD солвер для большей стабильности

            self.model.fit(X_train, y_train)
        except Exception as e:
            print(f"Ошибка при обучении модели: {e}")
            print("Попытка перезапуска с увеличенным параметром регуляризации alpha и модифицированными данными...")

            # Проверяем сингулярность входной матрицы
            try:
                s = np.linalg.svd(X_train, compute_uv=False)
                print(f"Минимальное сингулярное значение: {np.min(s)}")
                if np.min(s) < 1e-10:
                ...
                print(f"Ошибка при вычислении SVD: {e_svd}")
                # Добавляем небольшой шум в любом случае
                X_train = X_train + np.random.normal(0, 1e-5, X_train.shape)

            # Увеличиваем alpha для большей стабильности и используем solver 'svd'
            self.model = Ridge(alpha=500.0, solver='svd', max_iter=2000)
            self.model.fit(X_train, y_train)
        ...
        # Сохраняем важность признаков (коэффициенты модели)
...
def main(db_path):
    """
    Main training function using the new RidgeTradeModel.
...
def _run_backtest(model: RidgeTradeModel, df: pd.DataFrame):
    """
    Run simple backtest on test data to evaluate trading performance.
    """
    ...
    # Get actual next_close values — drop NaN (last row has no next_close)
    ...
    # Calculate signals (BUY if predicted > current, SELL otherwise)

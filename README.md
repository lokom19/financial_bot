...
### Настройка периода и интервала свечей

Скрипт `all_dfs_to_db.py` поддерживает аргументы командной строки:

```bash
# По умолчанию: 1000 дней, дневные свечи
python all_dfs_to_db.py

# Загрузить данные за последний год
python all_dfs_to_db.py --days 365

# 30 дней, часовые свечи
python all_dfs_to_db.py --days 30 --interval hour

# 5-минутные свечи за 100 дней
python all_dfs_to_db.py --days 100 --interval 5min

# Показать справку
python all_dfs_to_db.py --help
```
...
| `all_dfs_to_db.py` | Свечи за указанный период (по умолчанию: 1000 дней, дневные) | `all_dfs.{FIGI}` |
...
Интерактивный дашборд для просмотра торговых сигналов с системой грейдов надёжности.
...
Каждый торговый сигнал получает грейд (A-F), отражающий надёжность предсказания.

| Грейд | Надёжность | Рекомендация |
|:-----:|:----------:|:-------------|
| **A** | Отличная | Сильный сигнал, можно следовать |
| **B** | Хорошая | Надёжный сигнал |
| **C** | Средняя | Умеренная надёжность |
| **D** | Низкая | Слабый сигнал, требует осторожности |
| **F** | Очень низкая | Не рекомендуется следовать |

### Метрики для расчёта грейда

Грейд рассчитывается на основе взвешенной комбинации метрик:

| Метрика | Вес | Пороги (A/B/C/D) |
|---------|:---:|:-----------------|
| **Консенсус моделей** | 35% | ≥80% / ≥60% / ≥40% / ≥20% |
| **R² (качество модели)** | 25% | ≥0.7 / ≥0.5 / ≥0.3 / ≥0.1 |
| **Direction Accuracy** | 25% | ≥65% / ≥55% / ≥50% / ≥45% |
| **Win Rate (бэктест)** | 15% | ≥60% / ≥55% / ≥50% / ≥45% |

### Пример интерпретации

```
SBER - BUY сигнал
├── Консенсус: 43% (3/7 моделей) → Грейд C
├── R²: 0.65 → Грейд B
├── Direction: 58% → Грейд B
└── Общий грейд: C (средняя надёжность)
```

**Рекомендация:** Следуйте сигналам с грейдом **B** и выше для более надёжных решений.
